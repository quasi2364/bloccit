...
RSpec.describe PostsController, type: :controller do
  ...
  context "moderator CRUD on a post they don't own" do
    before do
    ...
    describe "GET show" do
      ...
      it "renders show view for post" do
        get :show, topic_id: my_topic.id, id: my_post.id
        expect(response).to render_template :show
      end
    ...
    describe "POST create" do
      ...
      it "assigns post attributes to @post" do
        post :create, topic_id: my_topic.id, post: {title: RandomData.random_sentence, body: RandomData.random_paragraph}
        expect(assigns(:post)).to eq Post.last
      end
    ...
    describe "GET edit" do
      it "returns http success - routes properly" do
        get :edit, topic_id: my_topic.id, id: my_post.id
        expect(response).to have_http_status(:success)
      end

      it "renders edit view - html view rendered" do
        get :edit, topic_id: my_topic.id, id: my_post.id
        expect(response).to render_template :edit
      end

      it "assigns correct post to edit to @post" do
        get :edit, topic_id: my_topic.id, id: my_post.id
        post_instance = assigns(:post)
        expect(post_instance.id).to eq my_post.id
        expect(post_instance.title).to eq my_post.title
    ...
    describe "PUT update" do
      it "it assigns new attribute values to @post" do
        new_title = RandomData.random_sentence
        new_body = RandomData.random_paragraph
        put :update, topic_id: my_topic.id, id: my_post.id, post: {title: new_title, body: new_body}
        updated_post = assigns(:post)
      ...
      it "increases count of Posts by 1" do
        expect{ put :update, topic_id: my_topic.id, id: my_post.id, post: {title: RandomData.random_sentence, body: RandomData.random_paragraph}}.to change(Post,:count).by(1)
      end
